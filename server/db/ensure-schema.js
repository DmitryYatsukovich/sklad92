...
import { syncAllUsersPermissionsFromRoles } from '../lib/sync-user-permissions-from-role.js';
...
const statements = [
  ...
  `UPDATE roles SET can_attendance_all = COALESCE(can_attendance, false)
   WHERE can_attendance_all IS NOT TRUE AND COALESCE(can_attendance, false) = true`,
  `UPDATE user_permissions SET can_attendance_all = COALESCE(can_attendance, false)
   WHERE can_attendance_all IS NOT TRUE AND COALESCE(can_attendance, false) = true`,
  ...
  `UPDATE roles SET can_attendance_edit = COALESCE(can_attendance, false)
   WHERE can_attendance_edit IS NOT TRUE AND COALESCE(can_attendance, false) = true`,
  `UPDATE user_permissions SET can_attendance_edit = COALESCE(can_attendance, false)
   WHERE can_attendance_edit IS NOT TRUE AND COALESCE(can_attendance, false) = true`,
  ...
  `UPDATE roles SET can_attendance_pay = COALESCE(can_attendance_all, false)
   WHERE can_attendance_pay IS NOT TRUE AND COALESCE(can_attendance_all, false) = true`,
  `UPDATE user_permissions SET can_attendance_pay = COALESCE(can_attendance_all, false)
   WHERE can_attendance_pay IS NOT TRUE AND COALESCE(can_attendance_all, false) = true`,
  ...
  `UPDATE roles SET can_attendance_edit_rates = true
   WHERE COALESCE(can_attendance_edit, false) AND COALESCE(can_attendance_pay, false)
     AND can_attendance_edit_rates IS NOT TRUE`,
  `UPDATE user_permissions SET can_attendance_edit_rates = true
   WHERE COALESCE(can_attendance_edit, false) AND COALESCE(can_attendance_pay, false)
     AND can_attendance_edit_rates IS NOT TRUE`,
  ...
  `UPDATE roles SET
     can_attendance_add_member = COALESCE(can_attendance_all, false),
     can_attendance_export = COALESCE(can_attendance_all, false),
     can_attendance_import = COALESCE(can_attendance_all, false)
   WHERE COALESCE(can_attendance_all, false) = true`,
  ...
  `UPDATE roles SET can_attendance_change_month = COALESCE(can_attendance, false)
   WHERE can_attendance_change_month IS NOT TRUE AND COALESCE(can_attendance, false) = true`,
  `UPDATE user_permissions SET can_attendance_change_month = COALESCE(can_attendance, false)
   WHERE can_attendance_change_month IS NOT TRUE AND COALESCE(can_attendance, false) = true`,
  `UPDATE roles SET
     can_settings_organizations = COALESCE(can_settings_organizations, false) OR COALESCE(can_settings, false),
     can_settings_warehouses = COALESCE(can_settings_warehouses, false) OR COALESCE(can_settings, false),
     can_settings_categories = COALESCE(can_settings_categories, false) OR COALESCE(can_settings, false),
     can_settings_work = COALESCE(can_settings_work, false) OR COALESCE(can_settings, false),
     can_roles = COALESCE(can_roles, false) OR COALESCE(can_users, false)
   WHERE COALESCE(can_settings, false) OR COALESCE(can_users, false)`,
  `UPDATE user_permissions SET
     can_settings_organizations = COALESCE(can_settings_organizations, false) OR COALESCE(can_settings, false),
     can_settings_warehouses = COALESCE(can_settings_warehouses, false) OR COALESCE(can_settings, false),
     can_settings_categories = COALESCE(can_settings_categories, false) OR COALESCE(can_settings, false),
