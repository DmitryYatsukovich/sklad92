...
const statements = [
  ...
  `ALTER TABLE roles ADD COLUMN IF NOT EXISTS can_offline_mode BOOLEAN DEFAULT true`,
  `ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_offline_mode BOOLEAN DEFAULT true`,
  ...
  `ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_face_all BOOLEAN DEFAULT false`,
  `ALTER TABLE roles ADD COLUMN IF NOT EXISTS can_face_all BOOLEAN DEFAULT false`,
  `ALTER TABLE user_permissions ADD COLUMN IF NOT EXISTS can_face_same_org BOOLEAN DEFAULT false`,
  `ALTER TABLE roles ADD COLUMN IF NOT EXISTS can_face_same_org BOOLEAN DEFAULT false`,
  ...
  `CREATE TABLE IF NOT EXISTS tools (
    ...
    type_id INTEGER NOT NULL REFERENCES tool_types(id) ON DELETE RESTRICT,
    serial_number VARCHAR(255) NOT NULL,
    purchase_date DATE,
    warranty_date DATE,
    cost DECIMAL(18,2) DEFAULT 0,
    status VARCHAR(20) NOT NULL DEFAULT 'new',
    object_id INTEGER REFERENCES warehouse_objects(id) ON DELETE SET NULL,
    ...
    repair_by_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
    repair_at TIMESTAMPTZ,
    repair_description TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CHECK (status IN ('new', 'in_use', 'in_repair', 'in_stock'))
  )`,
  `CREATE UNIQUE INDEX IF NOT EXISTS idx_tools_serial_number ON tools(serial_number)`,
  `CREATE INDEX IF NOT EXISTS idx_tools_type ON tools(type_id)`,
  `CREATE INDEX IF NOT EXISTS idx_tools_status ON tools(status)`,
  `CREATE INDEX IF NOT EXISTS idx_tools_location ON tools(object_id, warehouse_id, rack_id)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_tool_events_tool_id ON tool_events(tool_id, created_at DESC)`,
  `CREATE INDEX IF NOT EXISTS idx_tool_events_action ON tool_events(action)`,
