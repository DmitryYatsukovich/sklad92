...
/** Удалить все материалы склада и связанные выдачи (только администратор). */
router.delete('/all', requireAdmin, async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { rows: [{ c: matCount }] } = await client.query(
      'SELECT COUNT(*)::int AS c FROM materials',
    );
    if (matCount === 0) {
      await client.query('COMMIT');
      return res.json({ deleted: 0, issuances_deleted: 0 });
    }
    const issDel = await client.query('DELETE FROM issuances RETURNING id');
    const matDel = await client.query('DELETE FROM materials RETURNING id');
    await client.query('COMMIT');
    res.json({
      deleted: matDel.rowCount,
      issuances_deleted: issDel.rowCount,
    });
  } catch (e) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('DELETE /materials/all:', e);
    res.status(500).json({ error: 'Не удалось очистить склад' });
  } finally {
    client.release();
  }
});
...
router.post('/import/preview', requirePermission('can_warehouse'), upload.single('file'), async (req, res) => {
  if (!req.file?.buffer) {
    return res.status(400).json({ error: 'Выберите файл Excel (.xlsx)' });
  }
  let workbook;
  try {
    workbook = parseImportWorkbook(req.file.buffer);
  } catch (e) {
  ...
  try {
    ...
    const materialsPreview = workbook.materials.length
      ? await previewMaterialsImport(client, workbook.materials, lookups, validateLocation)
      : {
        total: 0, toCreate: 0, toUpdate: 0,
        groupsCreate: 0, groupsUpdate: 0, partsCreate: 0, partsUpdate: 0,
        singlesCreate: 0, singlesUpdate: 0, autoCreate: [], warnings: [],
        canImport: true,
      };
    const issuancesPreview = workbook.issuances.length
      ? await previewIssuancesImport(client, workbook.issuances, workbook.materials)
      : { total: 0, toCreate: 0, toUpdate: 0, warnings: [] };
    const productionPreview = workbook.production.length
      ? await previewProductionImport(
        client,
        workbook.production,
        workbook.issuances,
        workbook.materials,
      )
      : { total: 0, toUpdate: 0, warnings: [] };

    const warnings = [
      ...materialsPreview.warnings.map((w) => ({ ...w, sheet: w.sheet || 'Склад' })),
      ...issuancesPreview.warnings,
      ...productionPreview.warnings,
    ];

    res.json({
      total: materialsPreview.total + issuancesPreview.total + productionPreview.total,
      toCreate: materialsPreview.toCreate + issuancesPreview.toCreate,
      toUpdate: materialsPreview.toUpdate + issuancesPreview.toUpdate + productionPreview.toUpdate,
      groupsCreate: materialsPreview.groupsCreate,
      groupsUpdate: materialsPreview.groupsUpdate,
      partsCreate: materialsPreview.partsCreate,
      partsUpdate: materialsPreview.partsUpdate,
      singlesCreate: materialsPreview.singlesCreate,
      singlesUpdate: materialsPreview.singlesUpdate,
      issuancesCreate: issuancesPreview.toCreate,
      issuancesUpdate: issuancesPreview.toUpdate,
      productionUpdate: productionPreview.toUpdate,
      autoCreate: materialsPreview.autoCreate,
      warnings,
      canImport: warnings.length === 0
        && (workbook.materials.length + workbook.issuances.length + workbook.production.length) > 0,
    });
  } finally {
    client.release();
  }
...
router.post('/import', requirePermission('can_warehouse'), upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (workbook.materials.length) {
      const matResult = await importMaterialsFromExcel(client, {
        items: workbook.materials,
        userId: req.session.userId,
        lookups,
        validateLocation,
        uniqueCode,
      ...
      Object.assign(result, {
        created: matResult.created,
        updated: matResult.updated,
        created_groups: matResult.created_groups,
        updated_groups: matResult.updated_groups,
        created_parts: matResult.created_parts,
        updated_parts: matResult.updated_parts,
      });
      result.errors.push(...matResult.errors.map((e) => ({ ...e, sheet: e.sheet || 'Склад' })));
    }

    if (workbook.issuances.length) {
      const issResult = await importIssuancesFromExcel(
        client,
        workbook.issuances,
        req.session.userId,
        { syncStock: workbook.materials.length > 0 },
      );
      result.issuances_created = issResult.created;
      result.issuances_updated = issResult.updated;
      result.created += issResult.created;
      result.updated += issResult.updated;
      result.errors.push(...issResult.errors);
    }

    if (workbook.production.length) {
      const prodResult = await importProductionFromExcel(
        client,
        workbook.production,
        req.session.userId,
      );
      result.production_updated = prodResult.updated;
      result.updated += prodResult.updated;
      result.errors.push(...prodResult.errors);
    }

    result.total = workbook.materials.length
      + workbook.issuances.length
      + workbook.production.length;
    res.json(result);
  } finally {
    client.release();
  }
...
async function loadMaterialsForExport(client, ids) {
  const idList = ids.map((id) => parseInt(id, 10)).filter((id) => id > 0);
  if (!idList.length) return [];
  const r = await client.query(
    `SELECT ${MATERIAL_SELECT}${MATERIAL_GROUP_SELECT_EXTRA}
     ${MATERIAL_FROM}
     ${MATERIAL_GROUP_JOINS}
     WHERE m.id = ANY($1::int[]) AND m.parent_material_id IS NULL
