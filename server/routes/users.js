...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [user.id, perms.can_warehouse, perms.can_issuance, perms.can_production, perms.can_users, perms.can_attendance]
    );
  ...
  try {
    ...
    await client.query(
      `INSERT INTO user_permissions (user_id, can_warehouse, can_issuance, can_production, can_users, can_attendance)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         can_warehouse = $2, can_issuance = $3, can_production = $4, can_users = $5, can_attendance = $6`,
      [id, perms.can_warehouse, perms.can_issuance, perms.can_production, perms.can_users, perms.can_attendance]
    );
