...
router.delete('/:id', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const issRows = (await client.query(
      'SELECT id, quantity, returned_quantity FROM issuances WHERE material_id = $1',
      [id],
    )).rows;
    ...
    for (const iss of issRows) {
      ...
      restoreTotal += issued - returned;
    }

    if (restoreTotal > 1e-9) {
      ...
      await logQuantityChange(client, {
        ...
        kind: 'delete_restore',
        note: `Отмена выдач перед удалением (${issRows.length})`,
      });
    }

    if (issRows.length) {
      await client.query('DELETE FROM issuances WHERE material_id = $1', [id]);
    }

    const del = await client.query('DELETE FROM materials WHERE id = $1 RETURNING id', [id]);
    if (!del.rowCount) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: 'Материал не найден' });
    }
