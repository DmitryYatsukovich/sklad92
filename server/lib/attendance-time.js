...
/** Подпись ячейки: первая отметка — время прихода; после ухода — часы между приходом и последней отметкой */
export function buildTimesheetCellLabel(checkInAt, checkOutAt, workedMinutes, dayComment) {
  const comment = dayComment != null ? String(dayComment).trim() : '';
  const hasComment = comment.length > 0;
  const hasIn = !!checkInAt;
  const hasOut = !!checkOutAt;
  const mins = workedMinutes != null ? Math.round(Number(workedMinutes)) : null;
  if (mins === 0 && !hasComment) return null;
  if (hasComment && !hasIn && !hasOut && (mins == null || mins <= 0)) {
    return '…';
  }
  if (!hasOut && mins == null) {
