...
/** Кто поставил приход/уход: отметка по лицу (face) или ручной ввод (manual) */
export function buildTimeAttribution(rec, which) {
  if (!rec) return null;

  const pack = (user, viaRaw) => {
    if (!user) return null;
    const name = user.name || user.login;
    if (!name) return null;
    const via = viaRaw === 'manual' ? 'manual' : 'face';
    return { name, via };
  };

  if (which === 'in') {
    if (!rec.check_in_at) return null;
    const via = rec.check_in_via;
    const user = joinUserPayload(rec, 'check_in_by_user_id', 'cib');
    if (user && via) return pack(user, via);
    const marked = joinUserPayload(rec, 'marked_by_user_id', 'mb');
    if (marked) return pack(marked, 'face');
    const edited = joinUserPayload(rec, 'edited_by_user_id', 'eb');
    if (edited) return pack(edited, 'manual');
    return null;
  }

  if (which === 'out') {
    if (!rec.check_out_at) return null;
    const via = rec.check_out_via;
    const user = joinUserPayload(rec, 'check_out_by_user_id', 'cob');
    if (user && via) return pack(user, via);
    const marked = joinUserPayload(rec, 'marked_by_user_id', 'mb');
    if (marked) return pack(marked, 'face');
    const edited = joinUserPayload(rec, 'edited_by_user_id', 'eb');
...
export function buildTimesheetDayPatch(rec) {
  ...
  return {
    ...
    check_in_by: buildTimeAttribution(rec, 'in'),
    check_out_by: buildTimeAttribution(rec, 'out'),
...
export const ATTENDANCE_DAY_SELECT = `
  SELECT a.id, a.user_id, a.visit_date, a.check_in_at, a.check_out_at,
         ...
         a.edited_by_user_id, a.edited_at, a.times_edited_at,
         a.check_in_by_user_id, a.check_out_by_user_id, a.check_in_via, a.check_out_via,
         ...
         cib.login AS cib_login, cib.first_name AS cib_first_name, cib.last_name AS cib_last_name,
         cib.display_name AS cib_display_name,
         cob.login AS cob_login, cob.first_name AS cob_first_name, cob.last_name AS cob_last_name,
         cob.display_name AS cob_display_name
  FROM attendance_records a
  LEFT JOIN users mb ON mb.id = a.marked_by_user_id
  LEFT JOIN users eb ON eb.id = a.edited_by_user_id
  LEFT JOIN users cib ON cib.id = a.check_in_by_user_id
  LEFT JOIN users cob ON cob.id = a.check_out_by_user_id`;
