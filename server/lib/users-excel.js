...
function parseBool(val) {
  ...
  if (s === '' || s === '-') return null;
  if (['да', 'yes', 'y', '1', 'true', '+', 'истина'].includes(s)) return true;
  if (['нет', 'no', 'n', '0', 'false', '-', 'ложь'].includes(s)) return false;
  return null;
}
...
function photoToExportBase64(facePhotoFilename) {
  if (!facePhotoFilename) return '';
  const filepath = facePhotoFilePath(facePhotoFilename);
...
export function buildTemplateBuffer() {
  ...
  const help = XLSX.utils.aoa_to_sheet([
    ...
    ['Права (да/нет)', 'Склад, Выдача, Выработка, Пользователи, Журнал, Настройка, Отметка'],
  ]);
...
export function userToExportRow(u) {
  ...
  return [
    ...
    '',
    photoToExportBase64(u.face_photo),
    face.json,
    face.count || '',
...
export async function buildExportBuffer(rows) {
  ...
  for (const u of rows) {
    ...
    row.height = u.face_photo ? 78 : 22;

    if (photoColIdx >= 0 && u.face_photo) {
      const filepath = facePhotoFilePath(u.face_photo);
      if (fs.existsSync(filepath)) {
        const buf = fs.readFileSync(filepath);
        const ext = u.face_photo.split('.').pop()?.toLowerCase() === 'png' ? 'png' : 'jpeg';
  ...
  if (isCreate) {
    ...
    const u = await client.query(
      `INSERT INTO users (login, password_hash, password_plain, display_name, first_name, last_name,
         ...
         phone, hourly_rate, role, role_id, internal_uid, face_descriptor)
