...
import { isQuickDeviceEnabled, prefetchOfflineData } from './lib/offlineCache';
...
export default function Layout({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    if (user && isQuickDeviceEnabled()) {
      const t = setTimeout(() => prefetchOfflineData(user).catch(() => {}), 400);
          ...
          <div className="flex items-center justify-between gap-2 h-9">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <div className="hidden xl:flex items-center gap-1.5 text-zinc-500 text-2xs font-mono">
                ...
                <span className={`w-1 h-1 rounded-full ${serverOnline ? 'bg-white' : 'bg-zinc-600'}`} title={serverOnline ? 'Сервер доступен' : 'Нет связи с сервером'} />
                {isQuickDeviceEnabled() && (
                  <span className="text-zinc-600" title="Кэш устройства для офлайн">· кэш</span>
                )}
              </div>
