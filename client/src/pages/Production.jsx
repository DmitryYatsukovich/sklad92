...
function enrichRow(r) {
  ...
  const unitSmr = Number(r.production_price) || 0;
  const smrTotal = Number(r.smr_total) ?? produced * unitSmr;
  return {
    ...r,
    _produced: produced,
    _unitSmr: unitSmr,
    _smrTotal: smrTotal,
    _userSearch: userLabel(r).toLowerCase(),
    _materialSearch: (r.material_name || '').toLowerCase(),
    _confirmed: !!r.production_confirmed,
    _workLocation: r.work_location_label || '',
  };
}
...
export default function Production({ user }) {
  ...
  }, [rows, filters, periodFrom, periodTo, isAdmin]);
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <span className="text-2xs text-zinc-500">
            {hasActiveFilters ? `${sortedList.length}/${rows.length}` : rows.length}
          </span>
      ...
      <div className="table-wrap">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              {sortedList.map((r) => (
                <tr key={r.issuance_id} className="border-b border-white/5 hover:bg-white/[0.02]">
        ...
        {rows.length > 0 && sortedList.length === 0 && !loading && (
