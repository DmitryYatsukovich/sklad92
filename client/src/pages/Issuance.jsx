...
export default function Issuance({ user }) {
  ...
  const enriched = useMemo(
    () => issuances.map((i) => enrichRow(i, materialPrices)),
    [issuances, materialPrices],
  );
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1">
          <span className="text-2xs text-zinc-500">
            {hasActiveFilters ? `${sortedList.length}/${issuances.length}` : issuances.length}
          </span>
          <button
            type="button"
      ...
      <div className="table-wrap">
        ...
        {issuances.length > 0 && sortedList.length === 0 && !loading && (
