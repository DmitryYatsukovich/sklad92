...
export default function Warehouse({ user }) {
  ...
  const handleDeleteMaterial = async () => {
    ...
    if (!window.confirm(`Удалить материал «${editing.name}» и все связанные выдачи?`)) return;
    setError('');
    try {
  ...
  return (
    <div className="space-y-2">
          ...
          <table className="table-compact">
            ...
            <tbody>
              {sortedList.map((m, rowIndex) => {
                ...
                >
                  <td className="text-center text-zinc-500 text-2xs tabular-nums">{rowIndex + 1}</td>
                  ...
                  <td className="text-right">
                    <button
                      ...
                      title="История изменений (клик)"
                    >
                      {qty}
                    </button>
                  </td>
                  <td className="text-right text-zinc-400 tabular-nums">{unitPrice.toFixed(2)}</td>
                  <td className="text-right text-white tabular-nums font-medium">{costTotal.toFixed(2)}</td>
                  <td className="text-right text-zinc-400 tabular-nums">{unitSmr.toFixed(2)}</td>
                  <td className="text-right text-zinc-300 tabular-nums">{smrTotalRow.toFixed(2)}</td>
      ...
      {showQrMaterial && (
        <MaterialQrModal material={showQrMaterial} onClose={() => setShowQrMaterial(null)} />
      )}
      ...
      {activeMaterial && activeStep === 'menu' && (
        <div className="modal-backdrop z-50" onClick={closeMaterialAction} role="dialog" aria-modal="true">
          <div className="card p-5 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-zinc-500 text-xs font-mono mb-3">{activeMaterial.code}</p>
