...
export default function AttendanceAll() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-white">Посещения сотрудников</h2>
      {error && <p className="text-red-400 text-sm">{error}</p>}

      <form onSubmit={apply} className="flex flex-wrap items-end gap-4">
        <div>
          <label className="block text-sm text-slate-400 mb-1">С даты</label>
          <input
            type="date"
            value={from}
        ...
        <div>
          <label className="block text-sm text-slate-400 mb-1">По дату</label>
          <input
            type="date"
            value={to}
        ...
        <button type="submit" className="px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-sm">
          Показать
        </button>
      </form>
