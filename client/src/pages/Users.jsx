...
function buildUserPayload(form, extras = {}) {
  const rateRaw = form.hourly_rate;
  let hourly_rate = null;
  if (rateRaw !== '' && rateRaw != null) {
    const cleaned = String(rateRaw).replace(/\s/g, '').replace(/₽/g, '').replace(',', '.');
    const n = Number(cleaned);
    if (!Number.isNaN(n) && n >= 0) hourly_rate = Math.round(n * 100) / 100;
  }
  const payload = {
    login: form.login,
    ...
    employment_org: form.employment_org,
...
export default function Users({ user }) {
  ...
  const openEdit = (u) => {
    ...
    setForm({
      ...
      phone: u.phone || '',
      hourly_rate: u.hourly_rate != null && u.hourly_rate !== '' ? String(u.hourly_rate) : '',
      role: u.role || 'user',
  ...
  const handleCreate = async (e) => {
    ...
    if (!pendingFaceDescriptor || pendingFaceDescriptor.length < 128) {
      return setError('Снимите шаблон лица для вкладки «Отметка»');
    }
    setError('');
    try {
      const payload = buildUserPayload(form, {
        face_descriptor: pendingFaceDescriptor,
      });
  ...
  const handleUpdate = async (e) => {
    ...
    const editUser = list.find((x) => x.id === editing);
    if (!editUser?.has_face && (!editFaceDescriptor || editFaceDescriptor.length < 128)) {
      setError('Снимите шаблон лица для вкладки «Отметка»');
      return;
    }
    setError('');
    setSaving(true);
    try {
      const payload = buildUserPayload(form, {
        ...(editFaceDescriptor?.length >= 128 ? { face_descriptor: editFaceDescriptor } : {}),
      });
      if (!editUser?.has_face || editFaceDescriptor?.length >= 128) payload.can_face = true;
  ...
  const sortedList = [...filteredList].sort((a, b) => {
    ...
    if (sortBy === 'hourly_rate') {
      va = va === '' || va == null ? -1 : Number(va);
      vb = vb === '' || vb == null ? -1 : Number(vb);
      if (Number.isNaN(va)) va = -1;
      if (Number.isNaN(vb)) vb = -1;
    } else {
  ...
  const formFields = (
    <>
      ...
      <div className="border border-amber-500/30 rounded-xl p-4 space-y-3 bg-amber-500/5">
        <div>
          <label className="label">Лицо для отметки <span className="text-red-400">*</span></label>
          <p className="text-2xs text-slate-400 mt-0.5">
            Обязательно снимите шаблон — без него пользователь не сможет отмечаться на вкладке «Отметка».
          </p>
        </div>
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="flex-1 min-w-[200px] space-y-2">
            ...
            {hasFaceTemplate && (
              <p className="text-emerald-400 text-sm">Шаблон лица готов к сохранению</p>
            )}
            {editing && editingUser?.has_face && !editFaceDescriptor && (
              <p className="text-slate-500 text-2xs">Сохранённый шаблон используется. Снимите заново, чтобы обновить.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder="руб/час"
        />
        <p className="text-slate-500 text-2xs mt-1">Стоимость работы сотрудника за один час, ₽</p>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-700 text-slate-400 text-sm">
                ...
                <th className="p-4 font-medium">
                  <button type="button" onClick={() => toggleSort('hourly_rate')} className="flex items-center gap-1 hover:text-white">
                    Ставка <SortIcon column="hourly_rate" />
                  </button>
                </th>
                <th className="p-4 font-medium">
            ...
            <tbody>
              {sortedList.map((u) => (
                <tr key={u.id} className="border-b border-slate-700/50 hover:bg-slate-800/30">
                  ...
                  <td className="p-4 text-slate-300 tabular-nums whitespace-nowrap">{formatRate(u.hourly_rate)}</td>
                  <td className="p-4">
