...
export default function Actions({ user }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const merged = mergeActions(local, serverRes.items || []);
      setPageCache('actions:list', merged);
      setItems(merged);
      refreshCounts();
    } catch (e) {
  ...
  useEffect(() => {
    const mem = peekPageCache('actions:list');
    if (mem?.length) {
      setItems(mem);
      setLoading(false);
    }
    load(Boolean(mem?.length));
    const unsub = subscribeActionLog(() => load(true));
    const t = setInterval(() => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        setError(r.error || 'Не удалось отправить действия на сервер');
        return;
      }
      await load();
    } catch (e) {
  ...
  const pagination = useListPagination(filtered, 'actions-page-size', filter);
