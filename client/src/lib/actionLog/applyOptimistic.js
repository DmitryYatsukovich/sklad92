...
function tempIssuanceId(entry) {
  if (entry.id != null) return -Number(entry.id) - 1_000_000;
  return -1_000_000_000;
}

function tempMaterialId(entry) {
  if (entry.id != null) return -Number(entry.id) - 2_000_000;
...
export function applyPendingToMaterials(materials, entries, ctx = {}) {
  const { catalog } = ctx;
  const list = materials.map((m) => ({ ...m, _pending: Boolean(m._pending) }));
...
export function applyPendingToIssuances(issuances, entries, ctx = {}) {
  let list = issuances.map((i) => ({ ...i, _pending: Boolean(i._pending) }));
...
function buildProductionRowFromIssue(entry, body, ctx) {
  ...
  return markPending({
    issuance_id: tempIssuanceId(entry),
    issued_at: issuedAt,
...
export function applyPendingToProduction(rows, entries, ctx = {}) {
  ...
  let list = rows.map((r) => ({ ...r, _pending: Boolean(r._pending) }));
  const removed = new Set();
